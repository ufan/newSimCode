...
#@
silicon_guard   = 1.036
long            = 0.200  + 2 * silicon_guard
inner           = 0.080  + 2 * silicon_guard
side            = 95.000 - 2 * silicon_guard
x_cad_offset    = -300.0


tungsten_coordinates =   [- long/2 - 4 * side - 3*inner,
                          - long/2 - 2 * side - 2*long,
                          - long/2 - 2 * side - 1*inner,
                          - long/2,
                            long/2,
                            long/2 + 2 * side + 1*inner,
                            long/2 + 2 * side + 2*long,
                            long/2 + 4 * side + 3*inner,
                          ] 
...
for vertexname in all_vertices_info.keys():
    ...
    if   z in all_z_thin:    
        i = all_x_thin.index(x)
        new_x = tungsten_coordinates[i] + x_cad_offset        
        i = all_y_thin.index(y)
        new_y = tungsten_coordinates[i]
    elif z in all_z_thick:    
        i = all_x_thick.index(x)
        new_x = tungsten_coordinates[i] + x_cad_offset     
        i = all_y_thick.index(y)
        new_y = tungsten_coordinates[i]
    else:
        raise Exception()
